      ...
      try {
        ...
        const { data: inventoryCheck } = await supabase
          ...
          try {
            ...
            // If no employees loaded, fetch from Supabase and cache locally
            if (!employeeList || employeeList.length === 0) {
              console.log('� Fetching employees from online database...');
              const { data: employees, error: employeeError } = await supabase
                .from('user_profiles')
                .select('id, full_name, email, role')
                .order('full_name', { ascending: true });
              try {
                ...
                if (!employeeList || employeeList.length === 0) {
                  ...
                  if (employees && employees.length > 0) {
                    // Store each employee in localDB
                ...
                // The rest of the original handleSave function remains unchanged
                console.log('🔄 Starting to save all data to database...');
