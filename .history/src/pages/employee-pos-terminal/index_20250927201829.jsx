...
const EmployeePOSTerminal = () => {
  ...
  const loadEmployees = () => {
    // First call the service to load employees
    ...
    try {
      ...
      // Server sync should only happen on Save Progress button
      // Keeping existing local data
      console.log('💾 Using local data only - sync will happen on Save Progress');
      
      // If we have no data at all, add a default test employee
      if (!employeeList.length) {
        ...
        // Store in IndexedDB and localStorage using promises
        localDB.storeEmployeeProfile(defaultEmployee)
          .then(() => {
            localStorage.setItem('cached_employees', JSON.stringify([defaultEmployee.full_name]));
            localStorage.setItem('cached_employee_options', JSON.stringify([{
              value: defaultEmployee.id,
              label: defaultEmployee.full_name
            }]));
          ...
          .catch(error => {
            console.error('Failed to store default employee:', error);
          });
      }
    } catch (err) {
