...
class POSService {
  // Create a new POS session with offline-first approach
  async createSession(employeeId, sessionDate = null) {
    try {
      if (!employeeId) {
        throw new Error('Employee ID is required to create a session');
      }

      const date = sessionDate || new Date().toISOString().split('T')[0];
      
      // Create session with client-side UUID first
      const session = {
        id: generateUUID(),
      ...
      console.log(`✅ Created offline session ${session.id} for employee ${employeeId}`);
      
      // Try server sync only if explicitly requested
      ...
      if (window.navigator.onLine) {
        try {
          const { data, error } = await supabase
            .from('pos_sessions')
            .insert([{
              id: session.id, // Use same UUID for consistency
              employee_id: employeeId,
              session_date: date,
              status: 'active'
            }])
          ...
          await localDB.markChangesAsSynced([session.id]);
          console.log(`✅ Synced session ${session.id} with server`);
        } catch (syncError) {
          console.warn(`⚠️ Server sync failed for session ${session.id}:`, syncError);
