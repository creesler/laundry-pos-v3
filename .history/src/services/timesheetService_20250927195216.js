...
class TimesheetService {
    ...
    try {
      ...
      const timesheet = {
        ...
        local_data: {
          ...
          clock_in_local_time: new Date().toLocaleTimeString()
        }
      };

      // Save to local storage by date for easy retrieval
      const storageKey = this.getLocalStorageKey(employeeId || employeeName);
      localStorage.setItem(storageKey, JSON.stringify(timesheet));
      
      // Also save to pending syncs collection
      const pendingSyncs = JSON.parse(localStorage.getItem('pending_timesheet_syncs') || '[]');
      pendingSyncs.push({
        id: localId,
        type: 'clock_in',
        data: timesheet,
      ...
      localStorage.setItem('pending_timesheet_syncs', JSON.stringify(pendingSyncs));
      
      console.log(`✅ Saved clock-in locally for ${employeeName || employeeId}`);
  ...
  async clockOut(employeeId, employeeName = null) {
    try {
      ...
      const minutesWorked = Math.round(hoursWorked * 60); // Convert to minutes for database
      ...
      const updatedTimesheet = {
        ...
        work_duration_minutes: minutesWorked,
        updated_at: now.toISOString(),
        is_synced: false,
        pending_sync: true,
        local_data: {
          ...timesheet.local_data,
          clock_out_local_time: now.toLocaleTimeString(),
          total_duration_display: `${Math.floor(hoursWorked)}h ${Math.round((hoursWorked % 1) * 60)}m`
        }
      };
      ...
      console.log(`✅ Saved clock-out locally for ${employeeName || employeeId}`);
  ...
  async syncTimesheets() {
    return withSessionRetry(async () => {
      ...
      // First ensure we have a valid employee ID
      for (const timesheet of unsynced) {
        try {
          let employeeId = timesheet.employee_id;
          
          // If we only have employee_name, try to find the ID
          if (!employeeId && timesheet.employee_name) {
            try {
              const { data: employee } = await supabase
                .from('user_profiles')
                .select('id')
                .ilike('full_name', timesheet.employee_name)
                .single();
              
              if (employee?.id) {
                employeeId = employee.id;
                console.log(`✅ Found employee ID for ${timesheet.employee_name}: ${employeeId}`);
              }
            } catch (e) {
              console.warn(`Could not find employee ID for ${timesheet.employee_name}:`, e);
            }
          }
          
          // Skip if we still don't have an employee ID
          if (!employeeId) {
            console.warn(`⚠️ Skipping timesheet - no valid employee ID for ${timesheet.employee_name || 'Unknown'}`);
            results.push({
              id: timesheet.id,
              success: false,
              error: 'No valid employee ID found'
            });
            continue;
          }
        try {
