  ...
  const [activeInput, setActiveInput] = useState(null);
    ...
    return () => {
      window.removeEventListener('beforeunload', handleBeforeUnload);
            ...
            const ticketsWithValidSession = tickets.map(ticket => ({
              // Only send fields required by schema and relationship
              ticket_number: ticket.ticketNumber ?? ticket.number ?? '',
              wash: Number(ticket.wash ?? 0),
              dry: Number(ticket.dry ?? 0),
              total: Number(ticket.total ?? 0),
              pos_session_id: validSessionId // Relationship: links ticket to session
            }));
            console.log('🟦 DEBUG: Ticket payload for Supabase:', JSON.stringify(ticketsWithValidSession, null, 2));

            // Save inventory items (after validSessionId is set)
            if (inventoryItems && inventoryItems.length > 0) {
              const inventoryItemsWithSession = inventoryItems.map(item => ({
                item_name: item.item_name ?? item.name ?? '',
                count: Number(item.count ?? 0),
                sales: Number(item.sales ?? 0),
                stock: Number(item.stock ?? 0),
                pos_session_id: validSessionId // Relationship: links inventory to session
              }));
              console.log('🟦 DEBUG: Inventory payload for Supabase:', JSON.stringify(inventoryItemsWithSession, null, 2));
              // await localDB.storeInventoryItems(inventoryItemsWithSession);
            }
    {
      id: 1,
      name: 'Downy 19 oz',
...
• Employee is currently clocked in`);
  ...
  let sessionId = currentSession?.id || null;
    try {
      ...
      // Session verification/creation block
      // ...existing code...

      // Now validSessionId is guaranteed to be set
      if (inventoryItems && inventoryItems.length > 0) {
        const inventoryItemsWithSession = inventoryItems.map(item => ({
          item_name: item.item_name ?? item.name ?? '',
          count: Number(item.count ?? 0),
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {!loadingEmployees && employeeList?.map((emp) => (
