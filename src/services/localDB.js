class LocalDB {
  constructor() {
    ...
    this.dbVersion = 3; // Bumped version to force upgrade and add missing stores
    this.db = null;
  ...
  initializeDB() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        // Tickets store (for offline tickets)
        if (!db.objectStoreNames.contains('posTickets')) {
          db.createObjectStore('posTickets', { keyPath: 'id', autoIncrement: true });
        }
        // Session store (for offline session)
        if (!db.objectStoreNames.contains('posSession')) {
          db.createObjectStore('posSession', { keyPath: 'id' });
        }
        // Ticket sequence
  ...
  // Tickets offline storage
  getAllTickets() {
    return new Promise((resolve) => {
      if (!this.db) return resolve([]);
      try {
        const tx = this.db.transaction('posTickets', 'readonly');
        const store = tx.objectStore('posTickets');
        const req = store.getAll();
        req.onsuccess = () => resolve(req.result || []);
        req.onerror = () => resolve([]);
      } catch (e) {
  ...
  storeTickets(tickets) {
    return new Promise((resolve, reject) => {
      if (!this.db) return reject(new Error('Database not initialized'));
      try {
        const tx = this.db.transaction('posTickets', 'readwrite');
        const store = tx.objectStore('posTickets');
        let completed = 0;
        tickets.forEach(ticket => {
          ...
          req.onsuccess = () => {
            completed++;
            if (completed === tickets.length) resolve();
          };
          req.onerror = () => reject(req.error);
        });
        if (tickets.length === 0) resolve();
      } catch (e) {
        reject(e);
      }
    });
  ...
  // Session offline storage
  getSession() {
    return new Promise((resolve) => {
      if (!this.db) return resolve(null);
      try {
        const tx = this.db.transaction('posSession', 'readonly');
        const store = tx.objectStore('posSession');
        const req = store.getAll();
        req.onsuccess = () => resolve(req.result && req.result.length > 0 ? req.result[0] : null);
  ...
  storeSession(session) {
    return new Promise((resolve, reject) => {
      if (!this.db) return reject(new Error('Database not initialized'));
      try {
        const tx = this.db.transaction('posSession', 'readwrite');
        const store = tx.objectStore('posSession');
