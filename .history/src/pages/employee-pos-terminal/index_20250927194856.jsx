...
const EmployeePOSTerminal = () => {
  ...
  // Load employees from database and update local storage
  const loadEmployees = async () => {
    try {
      setLoadingEmployees(true);
      setError(null);
      
      // Try to load from local storage first for faster initial render
      ...
      if (employees?.length > 0) {
        ...
        const nonAdminEmployees = employees.filter(emp => emp.role?.toLowerCase() !== 'admin');
        
        // Create employee names list and options list
        const employeeNames = nonAdminEmployees.map(emp => emp.full_name);
        const options = nonAdminEmployees.map(emp => ({
          value: emp.id,
          label: emp.full_name
        }));
        ...
        localStorage.setItem('cached_employees', JSON.stringify(employeeNames));
        localStorage.setItem('cached_employee_options', JSON.stringify(options));
        
        console.log(`✅ Loaded ${employeeNames.length} employees from database`);
      } else {
  ...
  // Enhanced employee change handler with offline-first session management
  const handleEmployeeChange = useCallback(async (newEmployeeName) => {
    ...
    try {
      ...
      // Find employee ID from options
      const employee = employeeOptions.find(emp => emp.label === newEmployeeName);
      if (!employee) {
        throw new Error('Employee not found in options');
      }
      ...
      // Create new offline session for employee
      const session = await posService.createSession(employee.value);
      setCurrentSession(session);
      console.log('✅ Created new session for', newEmployeeName);

      // Check for existing offline data
      const existingSession = await localDB.getActiveSession(employee.value);
      if (existingSession) {
        console.log('ℹ️ Found existing offline session for', newEmployeeName);
        // Restore offline data if exists
        setInventoryItems(existingSession.inventory || []);
        setTickets(existingSession.tickets || []);
        setCashData(existingSession.cash || { started: 0, added: 0, total: 0 });
...
✅ Session created and ready for offline use
ℹ️ All changes will be saved locally first

🔄 Fresh session initialized with:
• START stock levels preserved
• Fresh sequential tickets
• Cash register reset
• Notes cleared`);

    } catch (error) {
      console.error('Error changing employee:', error);
      alert(`Failed to switch to ${newEmployeeName}: ${error.message}

Please try again.`);
    } finally {
      setLoading(false);
    }
  }, [selectedEmployee, currentSession, hasUnsavedChanges, handleSave, employeeOptions]);

  // Enhanced save handler with offline-first approach
  const handleSave = async () => {
    try {
      ...
      // Get employee ID
      const employee = employeeOptions.find(emp => emp.label === selectedEmployee);
      if (!employee) {
        throw new Error('Employee not found in options');
      }
      ...
      if (window.navigator.onLine) {
        try {
          await posService.saveSessionProgress(currentSession.id, sessionData);
          console.log('✅ Session synced with server');
          
          alert(`✅ All data saved successfully for ${selectedEmployee}!

✅ Saved locally and synced with server

ℹ️ Data saved:
• Session ID: ${currentSession.id}
• Inventory items: ${inventoryItems.length}
• Tickets: ${tickets.filter(t => t?.ticketNumber).length}
• Cash total: $${((cashData?.started || 0) + (cashData?.added || 0))?.toFixed(2)}`);
        } catch (syncError) {
          console.warn('⚠️ Server sync failed:', syncError);
          
          alert(`✅ Data saved locally for ${selectedEmployee}

⚠️ Could not sync with server
• Your data is safe and will sync later
...
Your current work is preserved in the browser.`);
    } finally {
      setLoading(false);
    }
  };
...
• Employee is currently clocked in`);
  ...
  // Save all data to database and fetch updated information (see implementation below)
  // Using useCallback for proper dependency handling
      ...
      // 2. Save inventory stock changes by date if table exists
      if (tableChecks.hasInventory) {
        console.log('📦 Saving inventory stock changes by date...');
        const today = new Date().toISOString().split('T')[0];
        
        // First, log all current inventory items
        await fetchAndLogInventory();
        
        // Get all inventory items (not filtered by date)
        ...
        if (fetchError) {
          console.warn('⚠️ Could not fetch existing inventory for today:', fetchError.message);
        }
        
        const updates = [];
        
        // Debug: Log all inventory items we're trying to match
        console.log('🔄 Attempting to match inventory items...');
        console.log('📦 Local inventory items:', inventoryItems.map(i => ({
          name: i.name,
          normalized: i.name?.toLowerCase().replace(/\s+/g, ' ').trim()
        })));
        
        // Only update existing items, don't insert new ones
        inventoryItems.forEach(item => {
          // Normalize item names for comparison (case and space insensitive)
          ...
          // Debug: Log all existing items for comparison
          console.log('🔍 Existing inventory items in DB:');
          existingInventory?.forEach(dbItem => {
            const dbName = dbItem.item_name;
            const dbNormalized = normalizeName(dbName);
            console.log(`- '${dbName}' -> '${dbNormalized}'`);
          });
          
          // Find matching item by normalized name
          const existingItem = existingInventory?.find(i => {
            const match = normalizeName(i.item_name) === itemName;
            console.log(`Comparing '${normalizeName(i.item_name)}' with '${itemName}': ${match ? 'MATCH' : 'no match'}`);
            return match;
          });
          
          if (existingItem) {
            console.log(`✅ Found match for '${item.name}' (ID: ${existingItem.id})`);
            // Only update if the item exists
            updates.push({
              id: existingItem.id,
              item_name: item.name || '',
              price: item.price != null ? parseFloat(item.price) : 0.00,
              start_count: item.start != null ? parseInt(item.start) : 0,
              add_count: item.add != null ? parseInt(item.add) : 0,
              sold_count: item.sold != null ? parseInt(item.sold) : 0,
              left_count: item.left != null ? parseInt(item.left) : 0,
              total_amount: parseFloat(item.total || 0).toFixed(2),
              updated_at: new Date().toISOString()
            });
          } else {
            console.warn(`⚠️ Skipping non-existent inventory item: ${item.name}`);
          }
        });
        
        // Only process updates if there are any
        if (updates.length > 0) {
          ...
          console.log(`✅ Updated ${updates.length} inventory items for ${today}`);
        } else {
          console.log('ℹ️ No matching inventory items found to update');
        }
        
        if (updates.length === 0 && inserts.length === 0) {
          console.log('ℹ️ No inventory changes to save');
        }
      } else {
        console.log('ℹ️ Skipping inventory update - table not available');
      }

      // 3. Save wash & dry tickets if table exists
      if (tableChecks.hasTickets && tickets.some(t => t?.ticketNumber)) {
        ...
        try {
          ...
          if (!sessionId) {
            console.log('ℹ️ No active session found, checking for existing sessions...');
            ...
            if (sessionError) {
              throw new Error(`Failed to check for active sessions: ${sessionError.message}`);
            }
            
            if (activeSessions?.length > 0) {
              sessionId = activeSessions[0].id;
              console.log(`ℹ️ Found active session: ${sessionId}`);
              setCurrentSession(activeSessions[0]);
            } else {
              // Create a new session if none exists
              console.log('ℹ️ No active sessions found, creating a new one...');
              const now = new Date();
              const newSession = {
                employee_id: selectedEmployee || 'unknown',
                ...
                cash_total: (cashData?.started || 0) + (cashData?.added || 0),
              ...
              console.log('Session creation response:', { createdSession, createError });
                
              if (createError || !createdSession) {
                throw new Error(`Failed to create new session: ${createError?.message || 'Unknown error'}`);
              }
              
              // Verify the session exists right after creation
              ...
              console.log('Session verification:', { verifySession, verifyError });
              
              if (verifyError || !verifySession) {
                throw new Error(`Failed to verify session creation: ${verifyError?.message || 'Session not found after creation'}`);
              }
          ...
          if (!sessionId) {
            throw new Error('No valid session ID available for saving tickets');
          }
          
          // Now prepare ticket data with the valid session ID
          const ticketData = tickets
            .filter(t => t?.ticketNumber)
            .map(ticket => ({
              ticket_number: ticket.ticketNumber,
              wash_amount: parseFloat(ticket.washAmount) || 0,
              dry_amount: parseFloat(ticket.dryAmount) || 0,
              total_amount: (parseFloat(ticket.washAmount) || 0) + (parseFloat(ticket.dryAmount) || 0),
          ...
          if (ticketData.length === 0) {
            console.log('ℹ️ No valid tickets to save');
            return;
          }
          
          console.log(`💾 Attempting to save ${ticketData.length} tickets with session:`, sessionId);
...
📝 Your data is temporarily preserved in the browser.`);
  ...
  // Enhanced save handler with offline-first session management
...
• Cash total: $${((cashData?.started || 0) + (cashData?.added || 0))?.toFixed(2)}`);
      ...
      console.error('Error saving data:', error);
      setError(`Save failed: ${error.message}`);
      throw error; // Re-throw for employee change handler
    } finally {
      setLoading(false);
    }
  }, [selectedEmployee, currentSession, inventoryItems, tickets, cashData, notes, employeeOptions, setLoading, setError]);
                ...
                <div className="space-y-2">
                  ...
                  {!loadingEmployees && employeeList?.length === 0 && (
                    <p className="text-sm text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">
                      ...
                      No employees found. Using fallback list.
                    </p>
                  )}
                  {!loadingEmployees && employeeList?.length > 0 && (
