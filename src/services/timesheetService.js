...
class TimesheetService {
  ...
  async clockIn(employeeId) {
    try {
      // Enforce valid employeeId (UUID)
      if (!employeeId || typeof employeeId !== 'string' || employeeId.length < 10) {
        throw new Error('A valid employee ID is required for clock in.');
      }
  ...
  async clockOut(employeeId, employeeName = null) {
    try {
      // Enforce valid employeeId (UUID)
      if (!employeeId || typeof employeeId !== 'string' || employeeId.length < 10) {
        throw new Error('A valid employee ID is required for clock out.');
      }
    ...
    return withSessionRetry(async () => {
      ...
      for (const timesheet of unsynced) {
        try {
          ...
          const dbTimesheet = {
            // Only include id if it's a real UUID, not a local_ id
            ...(timesheet.id && !timesheet.id.startsWith('local_') ? { id: timesheet.id } : {}),
          ...
          // Debug log for employee_id and payload
          console.log('Syncing timesheet to Supabase:', { employee_id: dbTimesheet.employee_id, payload: dbTimesheet });
