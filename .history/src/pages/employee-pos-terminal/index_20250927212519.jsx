  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save employee_timesheet (clock in/out)
        const employeeToSave = employeeList.find(emp => emp.id === selectedEmployee);
        if (!employeeToSave || !employeeToSave.id) {
          throw new Error('Selected employee is missing a valid ID. Cannot save to localDB.');
        }
        await localDB.storeEmployeeProfile(employeeToSave);
      ...
      alert('✅ All data saved locally. Press Save Progress again to sync with server when online.');
          ...
          let validSessionId = currentSession?.id;
          if (!validSessionId) {
            console.log('ℹ️ No session ID found, creating a new one...');
            validSessionId = await createNewSession();
          } else {
            console.log('✅ Using existing session:', validSessionId);
          }
                  ...
                  {!loadingEmployees && employeeList?.length > 0 && (
                    ...
                    <div className="mt-4 flex justify-end">
                      <SaveProgressButton
                        onSave={handleSave}
                        loading={loading}
                        disabled={loadingEmployees || !selectedEmployee}
                      />
                    </div>
                </div>
                <div className="space-y-2">
